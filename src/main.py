...
pattern = re.compile(r"(https?://\w+?\.\w{2,}/.*?)\s(\d{1,2}:[0-5]?\d:[0-5]?\d\s){2}(\w*?)(\s[a-zA-Z]{2}:\w*)+")
...
def main():
    ...
    failed = 0
    all = 0
    with open("../bat", encoding='utf8') as f:
        for i, line in enumerate(f.readlines()):
            ...
            try:
                ...
                print(f"[   !   ]  Generating [{start} - {end}] @ {url} called {name}")
                uid = clipper.generate(url, start, end)
